...
export function ScheduleList() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      // Fetch events to find all schedule IDs owned by this address
      ...
      // Extract unique schedule IDs from Deposited events
      const ids = [
        ...new Set(
          (data.events || [])
  ...
  if (!isConnected) {
    return (
      <div className="text-center py-12 text-white/30 text-sm">
        Connect your wallet to view schedules.
      </div>
    );
  }
  ...
  if (SCHEDULER_ADDRESS === "0x0000000000000000000000000000000000000000") {
    return (
      <div className="text-center py-12 text-yellow-400/50 text-sm">
        Scheduler contract not deployed yet.
      </div>
    );
  }
              ...
              {active.map((s) => (
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    ...
                    <div>
                      <div className="text-white/30 mb-1">Remaining</div>
                      <div className="text-white font-mono">
                        {formatEther(
                          BigInt(s.amountPerRelease) *
                          (BigInt(s.occurrences) - BigInt(s.released))
                        )} ETH
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-3 bg-white/10 rounded-full h-1">
                    <div
                      ...
                      style={{
                        width: `${(Number(s.released) / Number(s.occurrences)) * 100}%`,
                      }}
                    />
                  </div>
                </motion.div>
